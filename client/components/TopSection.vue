<template>
  <div class="basis-1/12 lg:w-4/12 xl:w-4/12 sm:w-10/12 md:w-10/12">
    <div class="h-100 max-w-screen-xl flex-row flex">
      ...
      <!-- Theme section -->
      <button type="button basis-1/6" @click="emitDarkMode"
      ...
      <button type="button basis-1/6" @click="emitLightMode" v-if="colorMode.value == 'dark'">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="white"
          class="fill-white w-6 h-6">
          <path stroke-linecap="round" stroke-linejoin="round"
...
const emit = defineEmits(['darkMode', 'lightMode']);

const emitDarkMode = () => {
  emit('darkMode', props.colorMode);
}

const emitLightMode = () => {
  emit('lightMode', props.colorMode);
}
</script>
